...
export async function getTournament(tournamentId, tourId, battleId) {
  let tournament = projects[0].tournament.find(
    (project) => project.tournamentId === tournamentId,
  );
...
export async function loader({ params }) {
  const battle = await getTournament(
    params.tournamentId,
    params.tourId,
    params.battleId,
  );
      ...
      <table className="table_battle">
        ...
        <tbody>
          {project.battle.pok_comb.map((pok_comb) => (
            <tr>
              <th>
                <img className="icon_pok" src={pok_comb.imgA}></img>{" "}
                {pok_comb.PokA}
              </th>
              <th>
                <img className="icon_pok" src={pok_comb.imgB}></img>{" "}
                {pok_comb.PokB}
              </th>
              <th>
                <img className="icon_pok" src={pok_comb.imgW}></img>{" "}
                {pok_comb.winner}
              </th>
            </tr>
          ))}
